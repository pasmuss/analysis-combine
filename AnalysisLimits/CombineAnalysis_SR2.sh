for subrange in SR2
do
    limit_file=Hbb_limits_${subrange}_alluncert_exptoy_theo
	...
	mkdir "${subrange}_${mass}_alluncert_exptoy_theo"
	...
	mv datacard_Analysis_toy_${subrange}_${mass}GeV.* ${subrange}_${mass}_alluncert_exptoy_theo
	cd ${subrange}_${mass}_alluncert_exptoy_theo
	echo "Starting combine ${subrange} ${mass}"
	combine -M MultiDimFit --setParameters mask_SR=1,r=0 -n Hbb_${subrange} -m ${mass} datacard_Analysis_toy_${subrange}_${mass}GeV.root  --freezeParameters r --redefineSignalPOIs turnon_novoeff --saveWorkspace  --cminDefaultMinimizerStrategy 0
	echo "Snapshot generation $mass GeV, $subrange done"
	combine -M FitDiagnostics -t -1 -d higgsCombineHbb_${subrange}.MultiDimFit.mH${mass}.root --snapshotName MultiDimFit --setParameters mask_SR=0,r=0,par0=0.000232321,peak=270.117,slope_novoeff=0.0122963,tail=-0.878842,turnon_novoeff=281.955,width=51.8367,offsetTF=250 --freezeParameters offsetTF,par0,slopelinTF --redefineSignalPOIs r --setParameterRanges r=-5,5:tail=-10,-0.5:par0=-0.001,0.001 -n Hbb_${subrange} -m $mass --plots --cminDefaultMinimizerStrategy 0 -v 5 > FD.log
	combine -M FitDiagnostics -t -1 -d higgsCombineHbb_${subrange}.MultiDimFit.mH${mass}.root --snapshotName MultiDimFit --expectSignal 1 --setParameters mask_SR=0,r=0,par0=0.000232321,peak=270.117,slope_novoeff=0.0122963,tail=-0.878842,turnon_novoeff=281.955,width=51.8367,offsetTF=250 --freezeParameters offsetTF,par0,slopelinTF --redefineSignalPOIs r --setParameterRanges r=-5,5:tail=-10,-0.5:par0=-0.001,0.001 -n Hbb_${subrange} -m $mass --plots --cminDefaultMinimizerStrategy 0 -v 5 > FDSig.log
	echo "Asimov"
	combine -M AsymptoticLimits -t -1 -d higgsCombineHbb_${subrange}.MultiDimFit.mH${mass}.root --snapshotName MultiDimFit --redefineSignalPOIs r --setParameters mask_SR=0,r=0,par0=0.000232321,peak=270.117,slope_novoeff=0.0122963,tail=-0.878842,turnon_novoeff=281.955,width=51.8367,offsetTF=250 --freezeParameters offsetTF,par0,slopelinTF --cminDefaultMinimizerStrategy 0 -n Hbb_${subrange} -m ${mass} --setParameterRanges r=-5,5:tail=-10,-0.5:par0=-0.001,0.001 -v 5 &> Asimov.log
	cd ..
	root_name=`readlink -f "${subrange}_${mass}_alluncert_exptoy_theo/higgsCombineHbb_${subrange}.AsymptoticLimits.mH${mass}.root"`
	echo "$root_name" >> "$limit_file"
	echo "$mass GeV $subrange finished"
    done
    echo "Merge Combination output:"
    hadd -f combineMerge_${subrange}.root ${subrange}_${mass}_alluncert_exptoy_theo/higgsCombineHbb_${subrange}.AsymptoticLimits.mH*.root
