...
//	Histo with 2HDM results:
	string thdm_production = "private/CMSSW_10_2_13/src/Analysis/MssmHbb/SusHi/FullRun_100PerJob_AllTypesAndBosons"; //production_corseBins_cosB_A_-1_1_tanB_1-100 //
	string thdm_type = "type2";//MSSM-like, i.e. needed for models like mhmod-plus or those with negative mu values
	string thdm_fname = "/nfs/dust/cms/user/asmusspa/" + thdm_production + "/rootFiles/Histograms3D_" + thdm_type + "_mA_mH.root";
	TFile f(thdm_fname.c_str(),"read");
//	Define mass points:
	vector<double> mAs{300,350,400};
...
//	MSSM benchm,ark file
	...
	string mssm_bench = cmsswBase + "/src/Analysis/MssmHbb/macros/signal/mh125_muneg_3_13.root";
...
//		if(i==0) thdm.GetXaxis()->SetRangeUser(0.8,1.);
	...
	leg.Draw();
	can.Print( (cmsswBase + "/src/Analysis/MssmHbb/macros/pictures/test_other_scenarios/20210108-0900/" + boson + "_" + model + "_BRs_tanB_mA-" + to_string_with_precision(mA,4) + "_cB_A-" + cB_A + ".pdf").c_str() );
}
