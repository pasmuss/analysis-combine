...
if ($#argv<4) then
	...
	sed -i 's/SUBRANGE/SR3/g' bias_datacard.txt
...
combine bias_datacard.txt -M FitDiagnostics --plots -s $seed --setParameters pdf_index=$index_fit,peak=240,tail=-0.711103,width=57.801 --freezeParameters pdf_index,peak --setParameterRanges r=-10,10:tail=-1,-0.7:width=40,65 --toysFile higgsCombineTest.GenerateOnly.mH${mp}.${seed}.root -m $mp -t $ntoys --cminDefaultMinimizerStrategy=0
EOF
