...
void biasSR4(){
  ...
  RooRealVar mbb("mbb","m_{12}",500,2000);
  RooArgList vars(mbb);

  vector<string> masses = {"1000","1200","1400","1600"};
  vector<double> lumiscalefactors = {74.27,71.75,73.48,75.7};

  //Input files, Data CR
  TFile* f_bgfit = new TFile("/afs/desy.de/user/a/asmusspa/Documents/CMSSW_9_2_15/src/Analysis/Models/results/BG-CR-3j_SR4/SuperNovo0_limfrozen-2/500to2000/workspace/FitContainer_workspace.root","READ");
  ...
  for (int mass = 0; mass < masses.size(); mass++){
    ...
    TH1F* sighist = (TH1F*)f_signal_in->Get("m12_SR4_10GeV");
    ...
    RooRealVar offsetTF("offsetTF","offset of TF in y direction",0.17,0.1,0.7);
    RooRealVar slopelinTF("slopelinTF","Slope of linear part of TF",-2.65e-5,-6.6e-5,-1e-8);
    ...
    RooRealVar emlalpha("emlalpha","alpha",2e-9,0.0,5.0);
    ...
    //Output
    TFile *fOut = new TFile(("ws_analysis_SR4_toySR_" + masses[mass]  + "GeV.root").c_str(),"RECREATE");
    ...
    TH1F* h_signalregion = (TH1F*)datasignalregion -> createHistogram("h_signalregion",mbb,Binning(150,500,2000));
    ...
    //Plotting
    //RooPlot *plot = mbb.frame();
    //dataaltSR -> plotOn(plot);
    //altsr -> plotOn(plot,RooFit::LineColor(kGreen));
    //signalregion -> plotOn(plot,RooFit::LineColor(kBlue));
  ...
  // Testing TF:
  // --> SR = CR*TF
  // // --> generate with alternative TF = ext. erf.
  // // --> fit this with "actual" TF (erf/const/lin/lin)
  //
}
