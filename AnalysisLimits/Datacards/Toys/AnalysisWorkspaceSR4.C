...
int AnalysisWorkspaceSR4(){
  ...
  int normCR = 110888;//FR4
  int normSR = 16678;//FR4
  int rebin = 1;
  ...
  for (unsigned int mass = 0; mass < srmasses.size(); mass++){
    ...
    TFile* f_sr_in = new TFile("/afs/desy.de/user/a/asmusspa/Documents/CMSSW_9_2_15/src/Analysis/Tools/test/Outputdata/TF_bias/Pseudos_finer5to10GeVBins_1904.root","READ");
    TH1F* SRHist = (TH1F*)f_sr_in->Get("SRHist_SR4");//data_obs SR
    ...
    RooRealVar slopelinTF("slopelinTF","Slope of linear part of TF",-1.86e-4,-5e-4,3e-4);
